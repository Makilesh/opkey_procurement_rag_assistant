...
def is_low_confidence(chunks: list["RetrievedChunk"]) -> bool:
    """True when the best chunk is only weakly relevant — the top rerank score
    cleared the refusal gate but stayed below CLARIFY_MAX_SCORE. That's the
    signal that retrieval found *something* but isn't sure it's the right thing,
    so the caller should ask the user to clarify rather than answer from a shaky
    match. Well-posed questions rerank ~0.9+, so this only fires on vague ones."""
    if not chunks:
        return False
